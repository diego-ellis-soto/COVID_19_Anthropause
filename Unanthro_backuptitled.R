...
locs = read.csv('one_point_per_day_20221118_anno.csv') %>%
  # locs = fread('one_point_per_day_20221118_anno.csv', data.table=FALSE) %>%
  dplyr::mutate(
...
server <- function(session, input, output){
    ...
    if(input$Data_for_map %in% c('Geohash 6')){
      ...
      leaflet() %>%
        ...
        addLegend(position = "bottomright",
                  ...
                  labels = c("2019", "2020"),
                  title = "Years")
      # addLegend('bottomright', pal = pal, values = dat_id()$year, title = 'Year', opacity = 0.8 )
  ...
  output$mymap_species <- renderLeaflet({
    ...
    #           pal = ~pal_species_geo_3(),
    #           values = species_lvl_dat_id()$n_points,
    #           title = "N. locations geohash 3",
    #           opacity = 1) 
    
    # addLegend("bottomright", pal = ~pal_species_geo_3,
    #           values = ~geo_3_species()$n_points,
